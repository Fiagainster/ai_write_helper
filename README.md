...
- **智能文本重写**：划选任意文本，一键触发AI重写，AI将综合分析划词和文档内容进行全局优化
- **上下文感知**：AI会同时读取划词信息和目标文档内容，进行综合判断和重写
...
2. 按下Enter键触发AI处理
3. 观察应用窗口底部的状态栏，了解处理进度：
   - 「正在处理...」：AI正在分析和生成内容
   - 「成功：AI补写成功完成」：处理完成
   - 「错误：XXX」：处理过程中出现问题
4. AI进行综合分析后，对整个文档进行智能重写和优化
5. 重写后的内容将完整替换原文档
...
- **完整文档重写模式**：AI会对整个文档进行重写，确保内容的一致性和连贯性
...
3. **AI处理**：调用DeepSeek API，结合内置优化提示词和用户自定义主题提示词，进行综合分析和重写
4. **智能清理**：对AI生成的内容进行智能清理，移除不必要的标记和说明文本
5. **原子写入**：使用临时文件+原子替换的方式，安全地将重写后的内容写回目标文档
6. **状态反馈**：实时更新处理状态，让用户随时了解进度
...
- **monitor.py**: 实现全局文本划选监听和Enter键触发机制，负责捕获用户选中的文本。
- **api.py**: 封装DeepSeek API调用，处理请求构造、响应解析和错误重试。
- **document.py**: 提供文档读写服务，支持TXT、MD、DOCX等多种格式。

### UI模块 (ui/)

- **main_window.py**: 实现应用的主配置窗口，包含API设置、文档设置和主题提示词编辑功能。
