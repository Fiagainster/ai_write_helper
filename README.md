...
- **多种AI服务支持**：支持DeepSeek、豆包、Kimi、通义千问和Ollama本地模型，可根据需求选择
...
1. 启动应用后，将直接显示主配置窗口
2. 在配置界面中完成以下设置：
   - **AI服务选择**：从下拉菜单中选择您要使用的AI服务（DeepSeek、豆包、Kimi、通义千问或Ollama本地模型）
   - **API设置**：根据选择的AI服务，输入相应的API密钥或模型名称并验证
     - 对于Ollama，输入您已下载的模型名称（如llama3、gemma等）
...
- **多种AI服务支持**：新增支持DeepSeek、豆包、Kimi、通义千问和Ollama本地模型，可根据需求选择
...
3. **AI处理**：调用配置的AI服务（DeepSeek、豆包、Kimi、通义千问或Ollama本地模型），结合内置优化提示词和用户自定义主题提示词，进行综合分析和生成
4. **智能清理**：对AI生成的内容进行智能清理，移除不必要的标记和说明文本
5. **写入模式处理**：根据配置的写入模式（增量、全量或光标补写）处理生成的内容
...
├── services/                # 服务模块
│   ├── __init__.py
│   ├── api.py               # 多种AI服务通信服务（DeepSeek、豆包、Kimi、通义千问、Ollama）
│   ├── document.py          # 文档读写服务，支持TXT、MD、DOCX等格式
│   └── monitor.py           # 全局文本监听服务
├── ui/                      # 用户界面模块
│   ├── __init__.py
│   └── main_window.py       # 主配置窗口和最小化球组件
├── config/                  # 配置相关
│   └── __init__.py
└── utils/                   # 工具模块
    └── __init__.py
...
- **api.py**: 封装多种AI服务调用，包括DeepSeek、豆包、Kimi、通义千问和Ollama本地模型，处理请求构造、响应解析和错误重试。
- **document.py**: 提供文档读写服务，支持TXT、MD、DOCX等多种格式，实现增量、全量和光标补写三种写入模式。
